...
void Application::Run()
{
	...
	Cube<float> cube{};
	cube.transform.position = P3D{ -0.2f,-0.6f,-5.f };
	...
	camera = camera.translate({ 0.f, -1.5f, 1.2f }) * camera.rotationFromAxis<Math::RightAxis>(0.5f);
	Math::Mat4<float> model = Math::Mat4<float>::identity();   //model matrix

	ContextRenderer contextRenderer{ viewport.getMatrixProjection(), camera };
	...
	while (!m_window->isWindowShouldClose())
	{
		...
		plane.render(contextRenderer, cube.color);
		//triangle.render(pvm);
